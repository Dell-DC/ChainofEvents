// node_modules/gamepad-wrapper/lib/standard-mapping.js
...
var STANDARD_AXES = {
  ...
  THUMBSTICK_RIGHT_Y: "THUMBSTICK_RIGHT_Y"
  // Vertical axis for right stick (negative up/positive down)
};
var STANDARD_AXES_MAPPING = {
  THUMBSTICK_LEFT_X: 0,
  THUMBSTICK_LEFT_Y: 1,
...
// node_modules/gamepad-wrapper/lib/xr-standard-mapping.js
var XR_STANDARD_BUTTONS = {
  TRIGGER: "TRIGGER",
  SQUEEZE: "SQUEEZE",
  TOUCHPAD: "TOUCHPAD",
...
var XR_STANDARD_BUTTON_MAPPING = {
  TRIGGER: 0,
  SQUEEZE: 1,
  TOUCHPAD: 2,
  THUMBSTICK: 3,
...
var GamepadWrapper = class {
  ...
  getButtonIdx(buttonId) {
    const buttonIdx = this._gamepad.mapping == "standard" ? STANDARD_BUTTON_MAPPING[buttonId] : this._gamepad.mapping == "xr-standard" ? XR_STANDARD_BUTTON_MAPPING[buttonId] : null;
    if (buttonIdx == null) {
  ...
  getAxisIdx(axisId) {
    const axisIdx = this._gamepad.mapping == "standard" ? STANDARD_AXES_MAPPING[axisId] : this._gamepad.mapping == "xr-standard" ? XR_STANDARD_AXES_MAPPING[axisId] : null;
    if (axisIdx == null) {
  ...
  getButtonDownByIndex(buttonIdx) {
    if (this._buttons[buttonIdx]) {
      return this._buttons[buttonIdx].prevFrame.value <= this._buttonPressValueMin && this._buttons[buttonIdx].currFrame.value > this._buttonPressValueMin;
    } else {
      return false;
    }
  }
  ...
  getButtonUpByIndex(buttonIdx) {
    if (this._buttons[buttonIdx]) {
      return this._buttons[buttonIdx].prevFrame.value >= this._buttonPressValueMax && this._buttons[buttonIdx].currFrame.value < this._buttonPressValueMax;
    } else {
      return false;
    }
  }
  ...
  getButtonClickByIndex(buttonIdx) {
    if (this._buttons[buttonIdx]) {
      return this._buttons[buttonIdx].prevFrame.value <= this._buttonClickThreshold && this._buttons[buttonIdx].currFrame.value > this._buttonClickThreshold;
    } else {
      return false;
    }
  }
...
//# sourceMappingURL=gamepad-wrapper.js.map
